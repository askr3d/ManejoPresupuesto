...
namespace ManejoPresupuesto.Controllers
{
    public class TransaccionesController: Controller
    {
        ...
        public async Task<IActionResult> Mensual(int año)
        {
            ...
            {
                var transaccion = transaccionesAgrupadas.FirstOrDefault(x => x.Mes == mes + 1);
                var fechaReferencia = new DateTime(año, mes + 1, 1);
        ...
        public async Task<FileResult> ExportarExcelPorMes(int mes, int año)
        {
            ...
            var transaccciones = await repositorioTransacciones.ObtenerPorUsuarioId(
            ...
            return GenerarExcel(nombreArchivo, transaccciones);
        }
        ...
        {
            var usuaioId = servicioUsuarios.ObtenerUsuarioId();
            if (!ModelState.IsValid)
            {
                modelo.Cuentas = await ObtenerCuentas(usuaioId);
                modelo.Categorias = await ObtenerCategorias(usuaioId, modelo.TipoOperacionId);
                return View(modelo);
            }

            var cuenta = await repositorioCuentas.ObtenerPorID(modelo.CuentaId, usuaioId);
            if(cuenta is null)
            {
                return RedirectToAction("NoEncontrado", "Home");
            }
            var categoria = await repositorioCategorias.ObtenrPorId(modelo.CategoriaId, usuaioId);
            if(categoria is null)
            {
                return RedirectToAction("NoEncontrado", "Home");
            }

            modelo.UsuarioId= usuaioId;
        ...
        public async Task<IActionResult> Editar(TransaccionActualizacionViewModel modelo)
        {
            ...
            var categoria = await repositorioCategorias.ObtenrPorId(modelo.CategoriaId, usuarioId);
            if (categoria is null)
            {
                return RedirectToAction("NoEncontrado", "Home");
            }
            ...
            await repositorioTransacciones.Actualizar
                (transaccion, modelo.MontoAnterior, modelo.CuentaAnteriorId);
